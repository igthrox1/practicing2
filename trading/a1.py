        ...
        try:
            async with websockets.connect(url, ping_interval=20) as ws:
                ...
                async for msg in ws:
                    ...
                    # Continuous Time Sync (The "Technology")
                    global TIME_OFFSET
...
async def bybit_ws(symbol, state):
    url = "wss://stream.bybit.com/v5/public/linear"  # Real market prices
...
def binance_order(symbol, side, qty, step_size):
    try:
        ...
        r = requests.post(
            ...
            headers={"X-MBX-APIKEY": BINANCE_KEY},
            params=params,
            timeout=5  # AGGRESSIVE: 5 seconds for orders
        )
...
def get_bybit_entry_info(symbol):
    ...
    try:
        ...
        print(f"Bybit entry info error: {e}")
        return None
...
def bybit_sign(params, timestamp, request_body=""):
    ...
    if params:
       ...
       # Build signature payload - For POST requests, request_body must be JSON string
    recv_window = "5000"
    
    # For GET: param_str only, empty request_body
    # For POST: empty param_str, request_body only
...
# =========================
async def api_pnl_tracker(state, coin, exchanges):
    """Calculate PNL manually using mark price"""
    binance_symbol = coin + "USDT"
    bybit_symbol = coin + "USDT"
    
    await asyncio.sleep(1)  # Wait 1 sec after trade execution
    
    # Fetch position info ONCE
    binance_info = None
    bybit_info = None
    ...
    print("\n💰 Starting Manual PNL tracker (1 sec updates)...")
    ...
    while True:
        try:
            ...
            # Binance PNL
            if binance_info and binance_info["entry_price"]:
                mark_price = state["binance"]["price"]
                entry = binance_info["entry_price"]
                size = binance_info["size"]
                
                if binance_info["side"] == "LONG":
            ...
            # Bybit PNL
            if bybit_info and bybit_info["entry_price"]:
                mark_price = state["bybit"]["price"]
                entry = bybit_info["entry_price"]
                size = bybit_info["size"]
                
                if bybit_info["side"] == "Buy":  # Bybit uses "Buy"/"Sell"
                    pnl = (mark_price - entry) * size
                else:  # Sell
                    pnl = (entry - mark_price) * size
                
                state["bybit"]["pnl"] = pnl
                net_pnl += pnl  # ✅ Use 'pnl' (already calculated above)
...
async def main():
    ...
    if "binance" in exchanges:
        state["binance"] = {"price": 0, "funding": None, "next_ts": None, "pnl": None}
        tasks.append(binance_ws(coin + "USDT", state))
    if "bybit" in exchanges:
        state["bybit"] = {"price": 0, "funding": None, "next_ts": None, "pnl": None}
        tasks.append(bybit_ws(coin + "USDT", state))
    if "mexc" in exchanges:
        state["mexc"] = {"price": 0, "funding": None, "next_ts": None, "pnl": None}
                ...
                if (state["binance"]["next_ts"] and state["bybit"]["next_ts"] and
                    ...
                    if should_execute(state[exchange_to_check]["next_ts"], EXECUTION_TIME):
                        if sig and sig['spread'] >= MIN_SPREAD:
                            ...
                            # Start API-based PNL tracker (no need to check position_info)
                            print("💰 Starting API-based PNL tracking...")
                            asyncio.create_task(api_pnl_tracker(state, coin, exchanges))
    ...
    async def exit_watcher():
        ...
        while not exit_fired:
            ...
            if exit_mode == "1":
                ...
                # Custom time exit
                should_exit = should_execute(state[exchange_to_check]["next_ts"], EXIT_TIME)
    
            if should_exit:
                print(f"\n🚪 EXIT TIME REACHED! Now waiting for positive PNL...")
    
                # 🆕 NEW FEATURE: Wait for positive PNL before exiting
                while True:
                    current_pnl = state.get('net_pnl', None)
                    
                    if current_pnl is not None and current_pnl >= 0:
                        print(f"✅ PNL is positive/zero (${current_pnl:+.3f})! Closing positions now...")
                        break
                    elif current_pnl is not None:
                        # Show waiting message with current negative PNL
                        print(f"⏳ Waiting for positive PNL... Current: ${current_pnl:+.3f}", end='\r')
                ...
                if "binance" in exchanges and position_info["binance"]["entry_price"]:
                    ...
                    binance_order(binance_symbol, side_to_close, qty, binance_step)
                ...
                if "bybit" in exchanges and position_info["bybit"]["entry_price"]:
                    ...
                    bybit_order(bybit_symbol, side_to_close, qty, bybit_step)
