...
def audit_log(msg):
    """Print a detailed audit log with high-precision timestamp"""
    ...
    try:
        ...
        for server in ntp_servers:
            try:
                ...
                print(f"   System time: {datetime.fromtimestamp(system_time).strftime('%H:%M:%S.%f')[:-3]}")
                print(f"   NTP time:    {datetime.fromtimestamp(ntp_time).strftime('%H:%M:%S.%f')[:-3]}")
                print(f"   Offset:      {TIME_OFFSET:.3f}s {'(AHEAD)' if TIME_OFFSET > 0 else '(BEHIND)'}")
                
                if abs(TIME_OFFSET) > 1:
                    print(f"⚠️  WARNING: Your system clock is {abs(TIME_OFFSET):.1f} seconds {'ahead' if TIME_OFFSET > 0 else 'behind'}!")
            ...
            async with websockets.connect(url, ping_interval=20) as ws:
                ...
                async for msg in ws:
                    ...
                    if "@bookTicker" in stream:
                        ...
                        # Midpoint for general price tracking
                        state["binance"]["price"] = (state["binance"]["bid"] + state["binance"]["ask"]) / 2
                    
                    elif "@markPrice" in stream:
                        # Update Funding & Time Sync
...
def get_binance_entry_info(symbol):
    """Fetch entry price and position size from Binance"""
    try:
        params = binance_sign({
            "symbol": symbol,
            "timestamp": int(get_accurate_time()*1000)  # Use NTP-synced time
        ...
        r = requests.get(
            BINANCE_URL + "/fapi/v2/positionRisk",
            headers={"X-MBX-APIKEY": BINANCE_KEY},
            params=params,
            timeout=3
        )
        ...
        print(f"Binance entry info error: {e}")
        return None

def get_bybit_entry_info(symbol):
    """Fetch entry price and position size from Bybit"""
    try:
        timestamp = str(int(get_accurate_time() * 1000))
        params = {"category": "linear", "symbol": symbol}
        signature = bybit_sign(params, timestamp, "")
        
        # Construct URL with sorted parameters to match signature
        ...
        if r.status_code != 200:
            print(f"Bybit info status error: {r.status_code} - {r.text}")
            return None
        ...
        print(f"Bybit entry info exception: {e}")
        return None
...
async def limit_order_chaser(exchange, symbol, side, total_qty, state, step_size, tick_size):
...
async def api_pnl_tracker(state, coin, exchanges, pos_info):
    ...
    print("\n💰 Starting Manual PNL tracker (Position refresh every 2s)...")
    
    iteration = 0

    while True:
        try:
            # ✅ HIGH-FREQUENCY SYNC: If exit is triggered, fetch positions on EVERY 0.2s loop
            # Otherwise, fetch every 10 iterations (2s)
            is_exiting = state.get("exit_triggered", False)
            sync_now = True if is_exiting else (iteration % 10 == 0)

            if sync_now:
                if "binance" in exchanges:
                    new_info = get_binance_entry_info(binance_symbol)
                    if new_info: pos_info["binance"] = new_info
                if "bybit" in exchanges:
                    new_info = get_bybit_entry_info(bybit_symbol)
                    if new_info: pos_info["bybit"] = new_info
            ...
            if b_info and b_info.get("entry_price"):
                mark_price = state["binance"]["price"]
            ...
            if y_info and y_info.get("entry_price"):
                mark_price = state["bybit"]["price"]
                entry = y_info["entry_price"]
                size = y_info["size"]
                
                if y_info["side"] == "Buy":  # Bybit uses "Buy"/"Sell"
                    pnl = (mark_price - entry) * size
                else:  # Sell
                    pnl = (entry - mark_price) * size
                ...
                if (state["binance"]["next_ts"] and state["bybit"]["next_ts"] and
                    ...
                    if should_execute(state[exchange_to_check]["next_ts"], EXECUTION_TIME):
                        if sig and sig['spread'] >= MIN_SPREAD:
                            ...
                            # ✅ ULTRA-PRECISION USDT HEDGE: The "Balancer" Strategy
                            # 1. Calculate Bybit first (the "Master" constraint)
                            qty_y_ideal = (usdt * leverage) / state["bybit"]["price"]
                            pure_qty_y = round_step_size(qty_y_ideal, bybit_step["step_size"])
                            actual_usdt_y = pure_qty_y * state["bybit"]["price"]
                            
                            # 2. Sync Binance to the "Nearest Fit" of Bybit's dollar value
                            qty_b_ideal = actual_usdt_y / state["binance"]["price"]
                            ...
                            print(f"📐 Parity Search: Target={usdt*leverage}$")
                            print(f"🎯 Bybit (Master):  {pure_qty_y} coins (~${actual_usdt_y:.4f})")
                            print(f"🎯 Binance (Sync):  {pure_qty_b} coins (~${actual_usdt_b:.4f})")
                            print(f"⚖️ Final Delta:    ${abs(mismatch):.6f}")
                            ...
                            tasks_chaser = []
                            if sig["long"] == "binance":
                                tasks_chaser.append(limit_order_chaser("binance", binance_symbol, "BUY", pure_qty_b, state, binance_step["step_size"], binance_step["tick_size"]))
                            else:
                                tasks_chaser.append(limit_order_chaser("binance", binance_symbol, "SELL", pure_qty_b, state, binance_step["step_size"], binance_step["tick_size"]))
                            
                            if sig["long"] == "bybit":
                                tasks_chaser.append(limit_order_chaser("bybit", bybit_symbol, "Buy", pure_qty_y, state, bybit_step["step_size"], bybit_step["tick_size"]))
                            else:
                                tasks_chaser.append(limit_order_chaser("bybit", bybit_symbol, "Sell", pure_qty_y, state, bybit_step["step_size"], bybit_step["tick_size"]))
                            ...
                            if "binance" in exchanges:
                                for attempt in range(10):  # Try 10 times
                                    binance_info = get_binance_entry_info(binance_symbol)
                                    if binance_info:
                            ...
                            if "bybit" in exchanges:
                                for attempt in range(10):  # Try 10 times
                                    bybit_info = get_bybit_entry_info(bybit_symbol)
                                    if bybit_info:
            ...
            if should_exit:
                ...
                while True:
                    ...
                    if projected_pnl is None:
                        print("⏳ Waiting for valid price data...", end='\r')
                        await asyncio.sleep(0.01)
                        continue

                    if projected_pnl >= 0:
                        print(f"✅ PNL is positive (${projected_pnl:+.3f})! Closing now...")
                        break
                    else:
                        print(f"⏳ Waiting for positive PNL... Current: ${projected_pnl:+.3f}", end='\r')
                    
                    await asyncio.sleep(0.001)  # Check every 1ms with LIVE data
                ...
                if "binance" in exchanges and position_info["binance"]["entry_price"]:
                    ...
                    binance_order(binance_symbol, side_to_close, qty, binance_step["step_size"])
                ...
                if "bybit" in exchanges and position_info["bybit"]["entry_price"]:
                    ...
                    bybit_order(bybit_symbol, side_to_close, qty, bybit_step["step_size"])
                
                exit_fired = True
                print("✅ All positions closed!")
                print(f"💰 Final NET PNL: ${state.get('net_pnl', 0):+.3f}")
                break
